...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax'])) {
    ...
    switch ($_POST['action']) {
        ...
        case 'get_all_stations':
            $stmt = $db->prepare("SELECT lsa.*, lm.full_name, lm.phone, lm.next_due_date, lm.status as member_status FROM linkspot_station_addresses lsa LEFT JOIN linkspot_members lm ON lsa.current_user_id = lm.id ORDER BY lsa.station_code, lsa.desk_number");
            $stmt->execute();
            $result = $stmt->get_result();
            $allStations = [];
            ...
            echo json_encode(['success' => true, 'data' => $allStations]);
            break;
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .station-card:hover, .station-card-small:hover {
            transform: translateY(-5px);
            box-shadow: 0 8px 25px rgba(0,0,0,0.15);
        }
        
        .station-card.available, .station-card-small.available {
            background: linear-gradient(135deg, #d4edda 0%, #c3e6cb 100%);
            color: #155724;
            border-color: #b1d8b7;
        }
        
        .station-card.occupied, .station-card-small.occupied {
        ...
        .station-card.selected, .station-card-small.selected {
        ...
        .station-card.reserved, .station-card-small.reserved {
            background: linear-gradient(135deg, #fff3cd 0%, #ffeaa7 100%);
            color: #856404;
            border-color: #ffda6a;
        }
        
        .station-card.maintenance, .station-card-small.maintenance {
        ...
        .station-action-btn.release {
            background: var(--danger);
        }
        
        .station-action-btn.release:hover {
            background: #c82333;
            transform: scale(1.1);
        }
        ...
        <div class="main-content">
            ...
            <div id="tab-payments" class="tab-content" style="<?php echo $action === 'payments' ? 'display: block;' : 'display: none;'; ?>">
                ...
                <div class="collapsible-form" id="paymentForm">
                    ...
                    <div class="form-group">
                        ...
                        <div class="station-grid-small" id="stationGrid">
            ...
            <div id="tab-members" class="tab-content" style="<?php echo $action === 'members' ? 'display: block;' : 'display: none;'; ?>">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
                    <div class="form-toggle" onclick="toggleMemberForm()" id="memberToggle">
                        <i class="fas fa-plus"></i>
                    </div>
                ...
                <div class="collapsible-form" id="memberForm">
                    ...
                    <div class="form-group">
                        <label class="form-label">Assign Station (Optional)</label>
                        <div class="station-grid-small" id="newMemberStations">
                            <!-- Stations will be loaded here -->
                        </div>
                    </div>
                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        <div style="display: flex; gap: 10px;">
                            <button class="btn btn-primary btn-sm" onclick="loadAllStations()">
                                <i class="fas fa-sync"></i> Refresh
                            </button>
                        </div>
                    </div>
    ...
    <script>
        ...
        function togglePaymentForm() {
            ...
            if (form.classList.contains('expanded')) {
                ...
                if (document.getElementById('stationGrid').children.length === 0) {
                    loadStations();
                    loadMembersList();
                }
            }
        ...
        function toggleMemberForm() {
            ...
            if (form.classList.contains('expanded')) {
                ...
                // Load data if not already loaded
                if (document.getElementById('newMemberStations').children.length === 0) {
                    loadStationsForMember();
                }
            }
        }
        ...
        function loadStations() {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    if (grid) {
                        grid.innerHTML = '';
                        displayStationsSmall(data.data, grid, 'payment');
                    }
                }
            });
        }
        ...
        function loadStationsForMember() {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    if (grid) {
                        grid.innerHTML = '';
                        displayStationsSmall(data.data, grid, 'member');
                    }
                }
            });
        }
        
        // Display stations in small grid
        function displayStationsSmall(stations, container, type) {
            ...
            for (const [code, stationList] of Object.entries(grouped)) {
                stationList.sort((a, b) => parseInt(a.desk_number) - parseInt(b.desk_number)).forEach(station => {
                    const card = document.createElement('div');
                    card.className = 'station-card-small available';
        ...
        // Display all stations in large grid
        function displayAllStations(stations) {
            ...
            sortedCodes.forEach(code => {
                grouped[code].sort((a, b) => parseInt(a.desk_number) - parseInt(b.desk_number)).forEach(station => {
                    const card = document.createElement('div');
                    ...
                    let occupantInfo = '';
                    if (station.current_user_name) {
                        occupantInfo = `<div class="station-occupant" title="${station.current_user_name}">${station.current_user_name}</div>`;
                    }
                    
                    card.innerHTML = `
                        <div style="font-size: 20px; font-weight: 600;">${code}${station.desk_number}</div>
                        <div style="font-size: 12px; margin-top: 5px; padding: 3px 8px; border-radius: 12px; background: ${statusClass === 'available' ? '#d4edda' : statusClass === 'occupied' ? '#f8d7da' : statusClass === 'reserved' ? '#fff3cd' : '#d1ecf1'}; color: ${statusClass === 'available' ? '#155724' : statusClass === 'occupied' ? '#721c24' : statusClass === 'reserved' ? '#856404' : '#0c5460'}">
                            ${statusClass.charAt(0).toUpperCase() + statusClass.slice(1)}
                        </div>
                        ${occupantInfo}
                        <div class="station-actions">
                            <button class="station-action-btn view" onclick="viewStationDetails(${station.id}, event)">
                                <i class="fas fa-eye"></i>
                            </button>
                            ${station.status === 'occupied' ? `
                            <button class="station-action-btn release" onclick="releaseStation(${station.id}, event)">
                    ...
                    if (tbody) {
                        ...
                        data.data.forEach(station => {
                            ...
                            row.innerHTML = `
                                ...
                                <td>
                                    <button class="btn btn-danger btn-sm" onclick="releaseStationConfirm(${station.id})">
        ...
        // Load all stations for space management
        ...
        function loadRecentPayments() {
            // This would need an endpoint to fetch recent payments
                ...
                tbody.innerHTML = `
                    <tr>
                        <td colspan="6" style="text-align: center; padding: 30px; color: #6c757d;">
                            <i class="fas fa-info-circle"></i> Payment history would be displayed here
                        </td>
                    </tr>
                `;
            }
        ...
        function selectStationForPayment(stationId) {
            ...
            updateStationSelection('stationGrid', 'payment');
        }
        ...
        function selectStationForMember(stationId) {
            ...
            updateStationSelection('newMemberStations', 'member');
        }
        
        // Update station selection UI
        function updateStationSelection(gridId, type) {
            const cards = document.querySelectorAll(`#${gridId} .station-card-small`);
            cards.forEach(card => {
                card.classList.remove('selected');
                const stationId = parseInt(card.dataset.id);
                if ((type === 'payment' && stationId === selectedStation) || 
                    (type === 'member' && stationId === selectedStationForMember)) {
        ...
        // View station details in modal
        function viewStationDetails(stationId, event) {
            event.stopPropagation();
            
            // Find the station in the current data
            const stations = Array.from(document.querySelectorAll('#allStationsGrid .station-card')).map(card => {
                return {
                    id: parseInt(card.dataset.id),
                    code: card.querySelector('div').textContent,
                    status: card.querySelector('div:nth-child(2)').textContent.toLowerCase(),
                    occupant: card.querySelector('.station-occupant') ? card.querySelector('.station-occupant').textContent : 'None'
                };
            });
            ...
            // Load more details from server if needed, or use current data
            const modalBody = document.getElementById('stationModalBody');
            modalBody.innerHTML = `
                <div style="margin-bottom: 20px;">
                    <div style="display: flex; align-items: center; gap: 15px; margin-bottom: 15px;">
                        <div style="width: 60px; height: 60px; background: ${station.status === 'available' ? '#d4edda' : station.status === 'occupied' ? '#f8d7da' : station.status === 'reserved' ? '#fff3cd' : '#d1ecf1'}; border-radius: 10px; display: flex; align-items: center; justify-content: center; font-size: 24px; font-weight: 600;">
                            ${station.code}
                        </div>
                        <div>
                            <h4 style="margin: 0 0 5px 0;">Station ${station.code}</h4>
                            <div style="padding: 5px 12px; background: ${station.status === 'available' ? '#d4edda' : station.status === 'occupied' ? '#f8d7da' : station.status === 'reserved' ? '#fff3cd' : '#d1ecf1'}; color: ${station.status === 'available' ? '#155724' : station.status === 'occupied' ? '#721c24' : station.status === 'reserved' ? '#856404' : '#0c5460'}; border-radius: 20px; display: inline-block; font-size: 12px; font-weight: 500;">
                    ...
                    <div style="background: #f8f9fa; padding: 15px; border-radius: 8px; margin: 15px 0;">
                        <h5 style="margin: 0 0 10px 0; color: #495057;">Current Occupant</h5>
                        ${station.occupant !== 'None' ? `
                        <p style="margin: 0; font-size: 16px; font-weight: 500;">${station.occupant}</p>
                        <p style="margin: 5px 0 0 0; color: #6c757d; font-size: 14px;">
                            <i class="fas fa-phone"></i> Contact info would appear here
                        </p>
                        ` : `
                        <p style="margin: 0; color: #6c757d; font-style: italic;">No occupant assigned</p>
                        `}
                    </div>
                    
                    <div style="background: #f8f9fa; padding: 15px; border-radius: 8px; margin: 15px 0;">
                        <h5 style="margin: 0 0 10px 0; color: #495057;">Station Information</h5>
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
                            <div>
                                <p style="margin: 0; font-size: 12px; color: #6c757d;">Type</p>
                                <p style="margin: 5px 0 0 0; font-weight: 500;">Standard Desk</p>
                            </div>
                            <div>
                                <p style="margin: 0; font-size: 12px; color: #6c757d;">Location</p>
                                <p style="margin: 5px 0 0 0; font-weight: 500;">Main Area</p>
                            </div>
                            <div>
                                <p style="margin: 0; font-size: 12px; color: #6c757d;">Equipment</p>
                                <p style="margin: 5px 0 0 0; font-weight: 500;">Standard Setup</p>
                            </div>
                            <div>
                                <p style="margin: 0; font-size: 12px; color: #6c757d;">Last Cleaned</p>
                                <p style="margin: 5px 0 0 0; font-weight: 500;">Today</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div style="display: flex; gap: 10px; margin-top: 20px;">
                        <button class="btn btn-danger" onclick="releaseStationConfirm(${stationId}); closeModal()" style="flex: 1;">
        ...
        // Release station with confirmation
        function releaseStationConfirm(stationId) {
        ...
        function releaseStation(stationId, event = null) {
            ...
            .then(data => {
                if (data.success) {
                    alert('Station released successfully!');
                    loadOccupiedStations();
                    loadAllStations();
                    if (document.getElementById('stationGrid')) {
                        loadStations();
                    }
                    if (document.getElementById('newMemberStations')) {
                        loadStationsForMember();
                    }
                    closeModal();
                } else {
                    alert('Failed to release station: ' + data.message);
                }
            });
        }
